...
mod tests {
    ...
    #[test]
    fn logistic_kernel_properties() {
        let logistic = Logistic;

        assert_abs_diff_eq!(
            logistic.evaluate(1.0),
            logistic.evaluate(-1.0),
            epsilon = KERNEL_SYMMETRY_EPS
        );

        assert!(logistic.evaluate(0.0) > logistic.evaluate(1.0));
        assert!(logistic.evaluate(0.0) > logistic.evaluate(2.0));

        for x in [-5.0, -2.0, 0.0, 2.0, 5.0] {
            assert!(logistic.evaluate(x) >= 0.0);
        }

        assert_eq!(logistic.support(), None);
    }

    #[test]
    fn sigmoid_kernel_properties() {
        let sigmoid = Sigmoid;

        assert_abs_diff_eq!(
            sigmoid.evaluate(1.0),
            sigmoid.evaluate(-1.0),
            epsilon = KERNEL_SYMMETRY_EPS
        );

        assert!(sigmoid.evaluate(0.0) > sigmoid.evaluate(1.0));
        assert!(sigmoid.evaluate(0.0) > sigmoid.evaluate(2.0));

        for x in [-5.0, -2.0, 0.0, 2.0, 5.0] {
            assert!(sigmoid.evaluate(x) >= 0.0);
        }

        assert_eq!(sigmoid.support(), None);
    }

    #[test]
    fn kernel_type_as_kernel_conversion() {
        ...
        for kt in kernel_types {
            ...
            assert_abs_diff_eq!(boxed.variance(), kt.variance(), epsilon = KERNEL_EXACT_EPS);
            assert_abs_diff_eq!(
                boxed.efficiency(),
                kt.efficiency(),
                epsilon = KERNEL_EXACT_EPS
    ...
    #[test]
    fn kernel_clone_functionality() {
        let original = KernelType::Tricube;
        ...
        let custom = CustomKernel::new(|x| (-x.abs()).exp())
            .with_variance(2.0)
            .with_efficiency(0.9);
        let custom_type = KernelType::Custom(Box::new(custom));
        let custom_cloned = custom_type.clone();

        assert_abs_diff_eq!(
            custom_type.evaluate(0.5),
            custom_cloned.evaluate(0.5),
            epsilon = KERNEL_EXACT_EPS
        );
    }

    #[test]
    fn kernel_clone_box_trait_object() {
        let gaussian = Gaussian;
        let boxed = gaussian.clone_box();

        assert_abs_diff_eq!(
            gaussian.evaluate(0.5),
            boxed.evaluate(0.5),
            epsilon = KERNEL_EXACT_EPS
        );

        let epanechnikov = Epanechnikov;
        let boxed2 = epanechnikov.clone_box();

        assert_abs_diff_eq!(
            epanechnikov.evaluate(0.5),
            boxed2.evaluate(0.5),
            epsilon = KERNEL_EXACT_EPS
        );
    }
    ...
    #[test]
    fn custom_kernel_default_metadata() {
        let custom = CustomKernel::new(|x| if x.abs() < 1.0 { 1.0 } else { 0.0 });

        assert_abs_diff_eq!(custom.variance(), 1.0, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom.efficiency(), 1.0, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom.bandwidth_factor(), 1.0, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom.roughness(), 0.0, epsilon = KERNEL_EXACT_EPS);
        assert_eq!(custom.support(), None);
    }

    #[test]
    fn kernel_type_name_coverage() {
        assert_eq!(KernelType::Gaussian.name(), "Gaussian");
        assert_eq!(KernelType::Epanechnikov.name(), "Epanechnikov");
        assert_eq!(KernelType::Triangular.name(), "Triangular");
        assert_eq!(KernelType::Tricube.name(), "Tricube");
        assert_eq!(KernelType::Bisquare.name(), "Bisquare");
        assert_eq!(KernelType::Uniform.name(), "Uniform");
        assert_eq!(KernelType::Cosine.name(), "Cosine");
        assert_eq!(KernelType::Logistic.name(), "Logistic");
        assert_eq!(KernelType::Sigmoid.name(), "Sigmoid");

        let custom = CustomKernel::new(|x| x);
        assert_eq!(KernelType::Custom(Box::new(custom)).name(), "Custom");
    }

    #[test]
    fn kernel_evaluate_with_bandwidth_all_kernels() {
        ...
        for kernel in kernels {
            ...
            let with_bandwidth = kernel.evaluate_with_bandwidth(x, h);
            let manual = kernel.evaluate(x / h) / h;

            assert_abs_diff_eq!(with_bandwidth, manual, epsilon = KERNEL_SYMMETRY_EPS);
        }
    }

    #[test]
    fn all_kernels_support_method() {
        assert_eq!(Epanechnikov.support(), Some((-1.0, 1.0)));
        assert_eq!(Triangular.support(), Some((-1.0, 1.0)));
        assert_eq!(Tricube.support(), Some((-1.0, 1.0)));
        assert_eq!(Bisquare.support(), Some((-1.0, 1.0)));
        assert_eq!(Uniform.support(), Some((-1.0, 1.0)));
        assert_eq!(Cosine.support(), Some((-1.0, 1.0)));

        assert_eq!(Gaussian.support(), None);
        assert_eq!(Logistic.support(), None);
        assert_eq!(Sigmoid.support(), None);
    }

    #[test]
    fn kernel_type_is_bounded_all_variants() {
        assert!(!KernelType::Gaussian.is_bounded());
        assert!(KernelType::Epanechnikov.is_bounded());
        assert!(KernelType::Triangular.is_bounded());
        assert!(KernelType::Tricube.is_bounded());
        assert!(KernelType::Bisquare.is_bounded());
        assert!(KernelType::Uniform.is_bounded());
        assert!(KernelType::Cosine.is_bounded());
        assert!(!KernelType::Logistic.is_bounded());
        assert!(!KernelType::Sigmoid.is_bounded());

        let custom_unbounded = CustomKernel::new(|x| (-x.abs()).exp());
        assert!(!KernelType::Custom(Box::new(custom_unbounded)).is_bounded());

        let custom_bounded =
            CustomKernel::new(|x| if x.abs() < 1.0 { 1.0 } else { 0.0 }).with_support((-1.0, 1.0));
        assert!(KernelType::Custom(Box::new(custom_bounded)).is_bounded());
    }

    #[test]
    fn triangular_kernel_complete_coverage() {
        let tri = Triangular;

        assert_eq!(tri.evaluate(1.0), 0.0);
        assert_eq!(tri.evaluate(-1.0), 0.0);
        assert_eq!(tri.evaluate(1.5), 0.0);
        assert_eq!(tri.evaluate(-1.5), 0.0);

        assert_abs_diff_eq!(tri.evaluate(0.0), 1.0, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(tri.evaluate(0.5), 0.5, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(tri.evaluate(-0.5), 0.5, epsilon = KERNEL_EXACT_EPS);
    }

    #[test]
    fn epanechnikov_kernel_complete_coverage() {
        let epan = Epanechnikov;

        assert_eq!(epan.evaluate(1.0), 0.0);
        assert_eq!(epan.evaluate(-1.0), 0.0);
        assert_eq!(epan.evaluate(1.5), 0.0);

        assert_abs_diff_eq!(epan.evaluate(0.0), 0.75, epsilon = KERNEL_EXACT_EPS);

        let val_half = 0.75 * (1.0 - 0.25);
        assert_abs_diff_eq!(epan.evaluate(0.5), val_half, epsilon = KERNEL_EXACT_EPS);
    }

    #[test]
    fn kernel_type_default() {
        let default_kernel = KernelType::default();
        assert_eq!(default_kernel.name(), "Gaussian");
    }

    #[test]
    fn custom_kernel_debug_format() {
        let custom = CustomKernel::new(|x| x.abs())
            .with_variance(2.5)
            .with_efficiency(0.95)
            .with_bandwidth_factor(1.8)
            .with_roughness(0.6)
            .with_support((-2.0, 2.0));

        let debug_str = format!("{:?}", custom);
        assert!(debug_str.contains("CustomKernel"));
        assert!(debug_str.contains("variance"));
        assert!(debug_str.contains("efficiency"));
    }

    #[test]
    fn normalize_weights_edge_cases() {
        let mut single = vec![5.0];
        normalize_weights(&mut single);
        assert_abs_diff_eq!(single[0], 1.0, epsilon = KERNEL_EXACT_EPS);

        let mut two = vec![3.0, 7.0];
        normalize_weights(&mut two);
        assert_abs_diff_eq!(two[0], 0.3, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(two[1], 0.7, epsilon = KERNEL_EXACT_EPS);
    }

    #[test]
    fn robust_reweights_edge_cases() {
        let residuals = vec![0.0, 0.0, 0.0];
        let weights = robust_reweights(&residuals, 1.0, Some(6.0));
        assert!(weights.iter().all(|&w| w == 1.0));

        let large_residuals = vec![10.0, 20.0, 30.0];
        let weights = robust_reweights(&large_residuals, 1.0, Some(6.0));
        assert!(weights.iter().all(|&w| w == 0.0));

        let residuals = vec![1.0, 2.0];
        let weights_default = robust_reweights(&residuals, 1.0, None);
        assert!(weights_default.len() == 2);
    }
    ...
    fn validate_bandwidth_all_edge_cases() {
        ...
        let result = catch_unwind(|| {
            validate_bandwidth(f64::NAN);
        });
        assert!(result.is_err());

        let result = catch_unwind(|| {
            validate_bandwidth(f64::INFINITY);
        });
        assert!(result.is_err());
    }
    ...
    fn validate_distance_nan_handling() {
        ...
        validate_distance(f64::INFINITY);
    }

    #[test]
    fn kernel_type_all_properties_coverage() {
        let kernels = vec![
            KernelType::Gaussian,
            KernelType::Epanechnikov,
            KernelType::Triangular,
        ...
        for kernel in &kernels {
            ...
            let _ = kernel.bandwidth_factor();
            let _ = kernel.variance();
            let _ = kernel.efficiency();
            let _ = kernel.roughness();
            let _ = kernel.name();
            let _ = kernel.is_bounded();
        }

        let custom = CustomKernel::new(|x| (-x.abs()).exp())
            .with_variance(1.5)
            .with_efficiency(0.88)
            .with_bandwidth_factor(1.2)
            .with_roughness(0.4);
        let custom_kernel = KernelType::Custom(Box::new(custom));

        assert_abs_diff_eq!(
            custom_kernel.bandwidth_factor(),
            1.2,
            epsilon = KERNEL_EXACT_EPS
        );
        assert_abs_diff_eq!(custom_kernel.variance(), 1.5, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom_kernel.efficiency(), 0.88, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom_kernel.roughness(), 0.4, epsilon = KERNEL_EXACT_EPS);
    }

    #[test]
    fn kernel_type_evaluate_all_branches() {
        let test_value = 0.3;

        assert!(KernelType::Gaussian.evaluate(test_value) > 0.0);
        assert!(KernelType::Epanechnikov.evaluate(test_value) > 0.0);
        assert!(KernelType::Triangular.evaluate(test_value) > 0.0);
        assert!(KernelType::Tricube.evaluate(test_value) > 0.0);
        assert!(KernelType::Bisquare.evaluate(test_value) > 0.0);
        assert!(KernelType::Uniform.evaluate(test_value) > 0.0);
        assert!(KernelType::Cosine.evaluate(test_value) > 0.0);
        assert!(KernelType::Logistic.evaluate(test_value) > 0.0);
        assert!(KernelType::Sigmoid.evaluate(test_value) > 0.0);

        let custom = CustomKernel::new(|x| 1.0 - x);
        assert!(KernelType::Custom(Box::new(custom)).evaluate(test_value) > 0.0);
    }

    #[test]
    fn normalize_weights_empty_slice() {
    ...
    #[test]
    fn robust_reweights_with_default_tuning() {
        let residuals = vec![0.5, 1.5, 3.0, 15.0];
        let scale = 2.0;

        let weights = robust_reweights(&residuals, scale, None);
        assert_eq!(weights.len(), 4);
    ...
    #[test]
    fn custom_kernel_all_builder_methods() {
        let custom = CustomKernel::new(|x| if x.abs() < 1.0 { 0.5 } else { 0.0 })
            .with_variance(3.0)
            .with_efficiency(0.75)
            .with_bandwidth_factor(2.5)
            .with_roughness(0.8)
            .with_support((-2.0, 2.0));

        assert_abs_diff_eq!(custom.variance(), 3.0, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom.efficiency(), 0.75, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom.bandwidth_factor(), 2.5, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom.roughness(), 0.8, epsilon = KERNEL_EXACT_EPS);
        assert_eq!(custom.support(), Some((-2.0, 2.0)));
    }

    #[test]
    fn kernel_evaluate_weight_default_implementation() {
        let epanechnikov = Epanechnikov;
        ...
        let x = 0.5;
        assert_abs_diff_eq!(
            epanechnikov.evaluate_weight(x),
            epanechnikov.evaluate(x),
            epsilon = KERNEL_EXACT_EPS
        );
        assert_abs_diff_eq!(
            triangular.evaluate_weight(x),
            triangular.evaluate(x),
            epsilon = KERNEL_EXACT_EPS
        );
        assert_abs_diff_eq!(
            logistic.evaluate_weight(x),
            logistic.evaluate(x),
            epsilon = KERNEL_EXACT_EPS
        );
        assert_abs_diff_eq!(
            sigmoid.evaluate_weight(x),
            sigmoid.evaluate(x),
            epsilon = KERNEL_EXACT_EPS
        );
    }
    ...
    #[test]
    fn compute_distance_weights_custom_kernel_unbounded() {
        let custom = CustomKernel::new(|x| (-x * x / 2.0).exp());
        let kernel = KernelType::Custom(Box::new(custom));
    ...
    #[test]
    fn compute_distance_weights_boundary_cases() {
        let tricube = KernelType::Tricube;

        let distances = vec![0.999, 1.0, 1.001];
        let weights = tricube.compute_distance_weights(&distances, 1.0);

        assert!(weights[0] > 0.0);
        assert_abs_diff_eq!(weights[1], 0.0, epsilon = KERNEL_EXACT_EPS); // Exactly at boundary
        assert_abs_diff_eq!(weights[2], 0.0, epsilon = KERNEL_EXACT_EPS); // Outside
    }

    #[test]
    fn kernel_type_clone_all_variants() {
        let kernels = vec![
            KernelType::Gaussian,
            KernelType::Epanechnikov,
            KernelType::Triangular,
    ...
    #[test]
    fn all_kernel_structs_clone_box() {
        let kernels: Vec<Box<dyn Kernel>> = vec![
            Box::new(Gaussian),
            Box::new(Epanechnikov),
            Box::new(Triangular),
    ...
    #[test]
    fn const_sqrt_helper_function() {
        const SQRT_4: f64 = const_sqrt(4.0);
        const SQRT_9: f64 = const_sqrt(9.0);
        const SQRT_0: f64 = const_sqrt(0.0);

        assert!((SQRT_4 - 2.0).abs() < 0.001);
        assert!((SQRT_9 - 3.0).abs() < 0.001);
        assert_eq!(SQRT_0, 0.0);
    }

    #[test]
    fn kernel_evaluate_fast_unbounded_kernels() {
        let gaussian = KernelType::Gaussian;
        let logistic = KernelType::Logistic;
        let sigmoid = KernelType::Sigmoid;
        ...
        for x in [0.0, 0.5, 1.0, 2.0, 10.0] {
            assert_abs_diff_eq!(
                gaussian.evaluate_fast(x),
                gaussian.evaluate(x),
                epsilon = KERNEL_EXACT_EPS
            );
            assert_abs_diff_eq!(
                logistic.evaluate_fast(x),
                logistic.evaluate(x),
                epsilon = KERNEL_EXACT_EPS
            );
            assert_abs_diff_eq!(
                sigmoid.evaluate_fast(x),
                sigmoid.evaluate(x),
                epsilon = KERNEL_EXACT_EPS
            );
        }
    ...
    #[test]
    fn normalize_weights_negative_values() {
        let mut negative = vec![-1.0, -2.0, -3.0];
        normalize_weights(&mut negative);

        let sum: f64 = negative.iter().sum();
        assert_abs_diff_eq!(sum, 1.0, epsilon = KERNEL_EXACT_EPS);
    }

    #[test]
    fn custom_kernel_partial_builder() {
        let custom1 = CustomKernel::new(|x| x.abs()).with_variance(2.0);
        assert_abs_diff_eq!(custom1.variance(), 2.0, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom1.efficiency(), 1.0, epsilon = KERNEL_EXACT_EPS); // default

        let custom2 = CustomKernel::new(|x| x.abs())
            .with_efficiency(0.9)
            .with_roughness(0.3);
        assert_abs_diff_eq!(custom2.efficiency(), 0.9, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom2.roughness(), 0.3, epsilon = KERNEL_EXACT_EPS);
        assert_abs_diff_eq!(custom2.variance(), 1.0, epsilon = KERNEL_EXACT_EPS); // default
    }

    #[test]
    fn all_bounded_kernels_zero_outside_support() {
        let bounded = vec![
            ("Epanechnikov", KernelType::Epanechnikov),
            ("Triangular", KernelType::Triangular),
            ("Tricube", KernelType::Tricube),
            ("Bisquare", KernelType::Bisquare),
            ("Uniform", KernelType::Uniform),
            ("Cosine", KernelType::Cosine),
        ];

        for (name, kernel) in bounded {
            for x in [1.0, 1.1, 2.0, 5.0, -1.0, -1.1, -2.0] {
                assert_eq!(
                    kernel.evaluate(x),
                    0.0,
                    "{} should be zero at x={}",
    ...
    #[test]
    fn robust_reweights_various_tuning_constants() {
        let residuals = vec![0.5, 1.0, 2.0, 5.0];
        let scale = 1.0;

        let weights_4 = robust_reweights(&residuals, scale, Some(4.0));
        let weights_6 = robust_reweights(&residuals, scale, Some(6.0));
        let weights_8 = robust_reweights(&residuals, scale, Some(8.0));

        assert!(weights_8[3] > weights_6[3]);
        assert!(weights_6[3] > weights_4[3]);
    }
}
