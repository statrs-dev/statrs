...
mod tests {
    ...
    fn test_large_samples() {
        let shorter = InfinitePeriodic::default(4.0, 1.0)
            .take(4 * 4096)
            .collect::<Vec<f64>>();
        let longer = InfinitePeriodic::default(4.0, 1.0)
            .take(4 * 32768)
            .collect::<Vec<f64>>();
        prec::assert_abs_diff_eq!((&shorter).mean(), 0.375, epsilon = 1e-14);
        prec::assert_abs_diff_eq!((&longer).mean(), 0.375, epsilon = 1e-14);
        prec::assert_abs_diff_eq!(
            (&shorter).quadratic_mean(),
            (0.21875f64).sqrt(),
            epsilon = 1e-14
        );
        prec::assert_abs_diff_eq!(
            (&longer).quadratic_mean(),
            (0.21875f64).sqrt(),
            epsilon = 1e-14
        );
    }
    ...
    fn test_quadratic_mean_of_sinusoidal() {
        let data = InfiniteSinusoidal::default(64.0, 16.0, 2.0)
            .take(128)
            .collect::<Vec<f64>>();
        prec::assert_abs_diff_eq!(
            (&data).quadratic_mean(),
